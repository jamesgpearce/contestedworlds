...
const source = await readFile(new URL('../lib/theme.ts', import.meta.url), 'utf8');
...
function environment(stored = null, systemDark = false, blocked = false) {
  ...
  let systemListener;
  const media = {matches:systemDark, addEventListener(_, listener){systemListener=listener;},removeEventListener(){systemListener=undefined;}};
  const root = {dataset:{},style:{},classList:{toggle(_,value){root.dark=value;}}};
  const localStorage = {getItem(key){if(blocked)throw new Error('Denied');return values.get(key)??null;},setItem(key,value){if(blocked)throw new Error('Denied');values.set(key,value);}};
  const window = {localStorage,matchMedia:()=>media,addEventListener(name,fn){listeners.set(name,fn);},removeEventListener(name){listeners.delete(name);},dispatchEvent(event){listeners.get(event.type)?.(event);}};
  const context = {exports:{},document:{documentElement:root},window,localStorage,matchMedia:window.matchMedia,Event};
  vm.runInNewContext(javascript,context);
  const api=context.exports;
  vm.runInNewContext(api.themeBootstrap,context);
  return {api,root,values,system(value){media.matches=value;systemListener?.();},storage(value){window.dispatchEvent({type:'storage',key:'caribbean-atlas-theme',newValue:value});}};
}
...
test('First paint respects system preference and an explicit saved override',()=>{
  ...
  assert.equal(environment('invalid',true).root.dataset.themePreference,'system');
});
test('A manual choice persists and is used on the next page load',()=>{
  ...
  assert.equal(environment(env.values.get('caribbean-atlas-theme')).root.dark,true);
});
