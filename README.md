...
| `detail` | A stable event ID, or `island-id.initial` for an initial period | No pinned card |

For example, `?islands=dominica&detail=dominica.initial` opens its initial Indigenous period, while `detail=saint-lucia-12` identifies a dated record. Targets use record identities rather than array positions or rounded dates, including clipped periods and claim cards. Removing the target island, excluding its period, hiding its claim marker, or passing an unknown record clears the pin.
