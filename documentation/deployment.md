...
| Variable         | Purpose                                                                |
| ---------------- | ---------------------------------------------------------------------- |
| `VITE_SITE_URL`  | Full canonical URL, including a trailing slash and any project path    |
| `VITE_BASE_PATH` | Empty at a custom domain; `/contestedworlds` at the GitHub project URL |
| `VITE_GA_ID`     | GA4 measurement ID; set to an empty string to disable collection       |
...
```sh
VITE_BASE_PATH=/contestedworlds \
VITE_SITE_URL=https://jamesgpearce.github.io/contestedworlds/ \
VITE_GA_ID= \
npm run build
VITE_BASE_PATH=/contestedworlds \
VITE_SITE_URL=https://jamesgpearce.github.io/contestedworlds/ \
...
The runtime history and coastline JSON use hashed URLs under `assets/`, independently of the readable exports under `data/`. The small bootstrap fetches them concurrently while the preloaded chart code and stylesheet arrive. An inline loading screen can paint before those requests finish, and the bootstrap has a separate 5,000-byte gzip budget. Include the entire generated `docs/` directory when deploying so that the referenced chunks and datasets arrive together.
...
The page-view commands are queued once when the Analytics component mounts. The tag download waits for the window's `load` event, a 1.5-second grace period, and then an idle callback with a 2-second timeout. Browsers without idle callbacks start after the grace period. These delays are our performance/measurement tradeoff; Google's standard tag snippet loads asynchronously in the head, while its [performance guidance](https://web.dev/articles/optimizing-content-efficiency-loading-third-party-javascript) supports waiting until critical content has loaded and warns that long delays can lose analytics data. Very short visits can still go unrecorded. Deferring the download does not remove the tag's bytes or guarantee that its eventual execution avoids long tasks.
...
Revert the offending source commit and push `main`; the workflow rebuilds and deploys that state. Source JSON, chart code and deployment settings remain versioned together.
